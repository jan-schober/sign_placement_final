...
def get_depth(camera_file_path, config):
    ...
    dictionary_with_lidar = get_lidar_points(piece_dictionary, lidar_df, threshold_lidar)

    # next step we print the lidar points on the camera image
    max_distance = config['max_distance']
    image_with_lidar_points = map_lidar_points_onto_image(camera_image_undist, dictionary_with_lidar, lidar_df, max_distance)
...
def slice_picture(mask, number_of_slices, H_PX, V_PX):
    ...
    for i in range(0, V_PX, piece_height):
        ...
        contours, hierachy = cv2.findContours(piece, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def get_lidar_points(dictionary, lidar_df, threshold):
    ...
    lidar_index_arr_R, depth_value_arr_R = get_closest_lidar_point(x_r_arr, y_arr, lidar_coordinates, lidar_df, threshold)
    lidar_index_arr_L, depth_value_arr_L = get_closest_lidar_point(x_l_arr, y_arr, lidar_coordinates, lidar_df, threshold)
...
def get_closest_lidar_point(x_arr, y_arr, lidar_coordinates, lidar_df, threshold):
    ...
    for x, y in zip(x_arr, y_arr):
        if x == -1:
            ...
            nearest_lidar_point = min(lidar_coordinates,
            ...
            if x_offset <= threshold and y_offset <= threshold:
                lidar_id = lidar_df[
                    (lidar_df['col'] == nearest_lidar_point[0]) & (lidar_df['row'] == nearest_lidar_point[1])].index
