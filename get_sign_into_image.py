...
def print_signs_on_image(
    camera_file_path: str,
    config_dict: dict,
    data: dict,
    lidar_data_path: str = None,
    img_with_lidar_points: np.ndarray = None,
):
    """Recommmends sign positions

    Args:
        camera_file_path (str): Filepath to the original camera image
        config_dict (dict): Your config dictionary
        data (dict): Dictionary of the lidar data
        lidar_data_path (str, optional): The path to the lidar data. Defaults to None, were the filepath is inferred from the config.
        img_with_lidar_points (np.ndarray, optional): An image with lidar points added, will generate an image of highlighted sign positions. Defaults to None.

    Returns:
        data_r_dic (dict): Data from the right street side
        data_l_dic (dict): Data from the left street side
        sign_dic_r (dict): Recommended Sign Positions right street side
        sign_dic_l (dict): Recommended Sign Positions left street side
        camera_image_with_signs (np.ndarray, optional) An cv2 image with recommended sign positons highlighted
    """
    ...
    if lidar_data_path is None:
        _, lidar_data_path = get_paths(camera_file_path)
    ...
    sign_dic_r, sign_dic_l = get_sign_dic(
        data_r_dic,
        data_l_dic,
        config_dict["camera_settings"],
        lidar_data_path,
        sign_size,
        free_space_dic,
    )
    if img_with_lidar_points is not None:
        ...
        camera_image_with_signs = print_sign(
            camera_image_with_signs, sign_img, sign_dic_l
        )
        return data_r_dic, data_l_dic, sign_dic_r, sign_dic_l, camera_image_with_signs
    return data_r_dic, data_l_dic, sign_dic_r, sign_dic_l
...
def print_sign(camera_image_undist, sign_img, sign_dic):
    ...
    """
    for x, y, w, h, flag in zip(
        sign_dic["x"], sign_dic["y"], sign_dic["w"], sign_dic["h"], sign_dic["flag"]
    ):
        if flag == -1:
...
def delete_wrong_points(data, max_distance):
    ...
    for i in range(0, len(data["depth_value_x_r"])):
        if (
            data["depth_value_x_r"][i] <= max_distance
            and data["depth_value_x_r"][i] != -1
        ):
        ...
        if (
            data["depth_value_x_l"][i] <= max_distance
            and data["depth_value_x_l"][i] != -1
        ):
...
def get_sign_dic(
    data_r_dict, data_l_dict, camera_dic, lidar_path, sign_size, free_space_dic
):
    """
    ...
    for depth_value, x_r, y_r, lidar_id in zip(
        data_r_dict["depth_value_x_r"],
        data_r_dict["x_r"],
        data_r_dict["y_r"],
        data_r_dict["lidar_id_r"],
    ):
        flag = check_free_space_right_side(
            lidar_id,
            lidar_df,
            beta_offset,
            gamma_offset,
        ...
        number_x_px, number_y_px = calculate_offset_px(
            depth_value, H_PX, V_PX, H_FOV_rad, V_FOV_rad
        )
        sign_width_px, sign_height_px = calculate_sign_size_px(
            depth_value, H_PX, V_PX, H_FOV_rad, V_FOV_rad, sign_size
        )
    ...
    for depth_value, x_l, y_l, lidar_id in zip(
        data_l_dict["depth_value_x_l"],
        data_l_dict["x_l"],
        data_l_dict["y_l"],
        data_l_dict["lidar_id_l"],
    ):
        flag = check_free_space_left_side(
            lidar_id,
            lidar_df,
            beta_offset,
            gamma_offset,
...
def check_free_space_right_side(
    lidar_id,
    lidar_df,
    beta_offset,
    gamma_offset,
    ...
    relevant_points = (
        (lidar_df["angle_alpha"] <= beta_angle)
        & (lidar_df["angle_alpha"] >= gamma_angle)
        & (p_2 + z_min <= lidar_df["p2"])
        & (lidar_df["p2"] <= p_2 + z_max)
    )
    ...
    for p_0_rel, p_1_rel in zip(relevant_points["p0"], relevant_points["p1"]):
        ...
        if c_distance_rel <= c_distance + c_depth_offset:
            lidar_id_point = lidar_df[
                (lidar_df["p0"] == p_0_rel) & (lidar_df["p1"] == p_1_rel)
            ].index
...
def check_free_space_left_side(
    lidar_id,
    lidar_df,
    beta_offset,
    gamma_offset,
