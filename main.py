...
for camera_file_path in file_name_list[::step_size_image]:
    ...
    final_dict, img_with_semantic_points, img_with_lidar_points = get_depth_street_edge.get_depth(camera_file_path,
                                                                                                  config_dict)
    ...
    if config_dict_used['debug_images']:
        debugging_image, c_sign, c_free_space, c_middle, c_out_image, number_signs = debugging_skript_sign_placement.main(
            camera_file_path, sign_r, sign_l, config_dict, data_r, data_l)
